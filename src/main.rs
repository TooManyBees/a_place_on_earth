...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    request_url.query_pairs_mut().append_pair("status", &message);
    ...
    let keys = {
        ...
        if !missing_env_vars.is_empty() {
            eprintln!("Missing required env vars:\n  {}", missing_env_vars.join("\n  "));
            std::process::exit(1);
        }
        ...
        Keys {
            ...
            secret_token: env_vars.remove("PLACE_ON_EARTH_SECRET_ACCESS_TOKEN").unwrap(),
        }
    };
    ...
    let mut res = client.post(request_url.as_str())
